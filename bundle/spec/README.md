...
pinned upstream tag and ship inside the skill bundle so consumers carry the canonical principle text alongside the skill
metadata.
...
| ------------------ | ---------------------------- | ------------------------------------------------------------- |
| `VERSION`          | `VERSION`                    | Spec version string; the skill's pinned `SPEC_VERSION`        |
...
MIT-licensed; vendoring a CC BY 4.0 source requires attribution only, satisfied by this README plus the upstream project
link in each principle's frontmatter `id` field.
